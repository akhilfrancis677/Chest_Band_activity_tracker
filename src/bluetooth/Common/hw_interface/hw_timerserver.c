...
uint16_t HW_TS_RTC_ReadLeftTicksToCount(void)
{
  ...
  if(LL_RTC_WAKEUP_IsEnabled(phrtc->Instance) == SET)
  {
    auro_reload_value = LL_RTC_WAKEUP_GetAutoReload(phrtc->Instance);
...
__weak void HW_TS_RTC_Int_AppNot(uint32_t eTimerProcessID, uint8_t ubTimerID, HW_TS_pTimerCb_t pfTimerCallBack)
{
